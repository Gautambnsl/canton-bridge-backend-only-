...
exports.CantonMinter = void 0;
const database_1 = require("../config/database");
const logger_1 = require("../config/logger");
const axios_1 = __importDefault(require("axios"));
const logger = new logger_1.Logger('CantonMinter');
class CantonMinter {
    constructor() {
    ...
    async mint(depositAddress, amount, cantonAddress, ethTxHash) {
        try {
            ...
            const cantonTxHash = await this.callCantonMintAPI(cantonAddress, amount, ethTxHash);
         ...
         WHERE address = $1`, [depositAddress]);
            throw err;
        }
    }
    async callCantonMintAPI(recipient, amount, ethTxHash) {
        try {
            ...
            const response = await axios_1.default.post(`http://${this.cantonGrpcEndpoint}/api/v1/mint`, mintPayload, {
                headers: {
    ...
    async getMintStatus(depositAddress) {
        try {
            const db = (0, database_1.getDB)();
            const result = await db.query(`SELECT status, canton_tx_hash, received_amount FROM deposit_addresses WHERE address = $1`, [depositAddress]);
...
exports.CantonMinter = CantonMinter;
//# sourceMappingURL=CantonMinter.js.map