...
const logger = new Logger('EVMMonitor');
...
export class EVMMonitor {
  ...
  async startMonitoring(depositAddresses: string[]): Promise<void> {
  ...
  private startPolling(depositAddresses: string[]): void {
  ...
  private async checkAddressForDeposits(depositAddress: string): Promise<void> {
    try {
      const db = getDB();
      ...
      for (const txRow of result.rows) {
        ...
        try {
          ...
          if (confirmations >= this.requiredConfirmations) {
            ...
            const depositAddressRecord = await db.query(
              `SELECT da.* FROM deposit_addresses da
               JOIN deposit_transactions dt ON da.id = dt.deposit_address_id
               WHERE dt.eth_tx_hash = $1`,
              [txHash]
            );
  ...
  async recordDeposit(
    ...
    try {
      ...
      await db.query(
        ...
        [txId, depositAddressId, txHash, amount, fromAddress, 0, 'PENDING']
      );
  ...
  private setupAlchemyWebhooks(depositAddresses: string[]): void {
